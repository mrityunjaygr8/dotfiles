...
--
-- Custom Functions
-- Function For showing system info
do
    local instance = nil
    systeminfo = function () 
        if instance then
...
-- Send Client(Window) to relative tag
function rel_send(rel_idx)
...
awful.screen.connect_for_each_screen(function(s)
    ...
    awful.tag({ "www", "code", "api", "misc"}, s, awful.layout.layouts[1])
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey, "Control" }, "Left",   function() rel_send(-1) end),
    awful.key({ modkey, "Control" }, "Right",  function() rel_send(1) end),
...
clientkeys = gears.table.join(
    ...
    awful.key({ modkey,           }, "o",      function () client_move_screen_relatively(1)               end),
    awful.key({ modkey, "Shift"   }, "o",      function () client_move_screen_relatively(-1)               end),
    ...
    awful.key({ "Ctrl", "Shift" }, "Escape", function () awful.util.spawn("xfce4-taskmanager")
	end),
	awful.key({ modkey }, "\\", systeminfo)
)
