...
return wibox.widget {
	...
	spacing = dpi(7),
	{
		...
		{
			require('widgets.brightness.brightness-slider'),
			bg = barColor,
			shape = function(cr, width, height)
		...
		{
			require('widgets.volume.volume-slider'),
			bg = barColor,
			shape = function(cr, width, height)
				gears.shape.partially_rounded_rect(cr, width, height, false, false, false, false, beautiful.groups_radius) end,
			forced_height = dpi(48),
		...
		{
			require('widgets.wifi.wifi-toggle'),
			bg = barColor,
			shape = function(cr, width, height)
				gears.shape.partially_rounded_rect(cr, width, height, false, false, false, false, beautiful.groups_radius) end,
			forced_height = dpi(48),
		...
		{
			require('widgets.bluetooth.bluetooth-toggle'),
			bg = barColor,
			shape = function(cr, width, height)
				 gears.shape.partially_rounded_rect(cr, width, height, false, false, false, false, beautiful.groups_radius) end,
			forced_height = dpi(48),
